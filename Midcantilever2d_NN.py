...
if run_mode == 1:
    ...
    prob.opt.options.outputs_path = 'outputs/Midcantilever2d_NN_Stress'
    ...
    prob.opt.parameters.target_volume = 0.5  # Match TOuNN default vf for TipCantilever
    prob.opt.options.outputs_path = 'outputs/Midcantilever2d_NN_Compliance'
    ...
    prob.opt.parameters.constraint_types = ['volume']
    prob.opt.parameters.target_volume = 0.5 
    prob.opt.options.outputs_path = 'outputs/Midcantilever2d_NN_MinStress'
    prob.opt.functions.objective_scale = 1.0
    prob.opt.functions.constraint_scale = 1.0
    prob.opt.options.move_limit = 0.05
elif run_mode == 3:
    ...
    prob.opt.options.outputs_path = 'outputs/Midcantilever2d_NN_LocalALM'
    prob.opt.functions.objective_scale = 20
    prob.opt.functions.constraint_scale = 1.0
    prob.opt.options.move_limit = 0.05
...
prob.opt.parameters.init_dens = 0.5
...
prob.solver = solver 
prob.fe.mesh_input.bcs_file = 'setup_midcantilever2d_bcs'
solver.init_FE()
